...
import { createCardCheckout, createOrder, isFirebaseConfigured } from '@pedido-listo/firebase';
...
export function CheckoutSheet({ business, items, subtotal, open, onClose }: Props) {
  ...
  function handleSend() {
    ...
    if (paymentMethod !== 'tarjeta' && !business.whatsapp?.replace(/\D/g, '')) {
    ...
    if (paymentMethod === 'tarjeta') {
      if (!business.id || !isFirebaseConfigured()) {
      ...
      void (async () => {
        try {
          savePendingCheckout({ checkout: { ...checkout, paymentStatus: 'paid' }, items });
          const { initPoint } = await createCardCheckout({
          ...
          setSending(false);
          alert(error instanceof Error ? error.message : 'No se pudo iniciar el pago con tarjeta.');
        }
      })();
      return;
    }
      ...
      <div className="sheet" role="dialog" aria-labelledby="checkout-title">
        ...
        <div className="form-section">
          ...
          {paymentMethod === 'tarjeta' && (
            <p className="clabe-hint">
              Vas a pagar en Mercado Pago. El dinero llega directo al negocio. Si cancelas, el pedido no se prepara.
            </p>
          )}
        ...
        <div className="actions">
          <button
            type="button"
            className={paymentMethod === 'tarjeta' ? 'btn-card' : 'btn-wa'}
            onClick={handleSend}
            disabled={sending}
          >
            {paymentMethod !== 'tarjeta' && (
